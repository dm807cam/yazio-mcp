 ...
 * client (see createFromCredentials) and connect the resulting `mcpServer` to
 * whichever transport the entrypoint chose.
 */
export class YazioMcpServer {
  private server: McpServer;
  private yazioClient: Yazio;
  ...
  static async createFromCredentials(credentials: YazioCredentials): Promise<YazioMcpServer> {
    return new YazioMcpServer(await createYazioClient(credentials));
  }

  /** The underlying MCP server, for connecting to a transport. */
...
- The tool sends: \`[{ date: "2025-12-18 12:00:00", water_intake: 750 }]\` to the API`
  ...
  private async ensureAuthenticated(): Promise<Yazio> {
    // The client is authenticated at construction time; this indirection is kept
    // so the ~15 tool methods below read unchanged from upstream.
